...
read_params(int argc, char **argv)
{
    ...
    if (argc < 2) {
        fprintf(stderr, "usage: %s -w workflow -d json_string [-s server]\n", argv[0]);
        exit(1);
    }
    ...
    const char* json_string)
{
    ...
    return mf_register_workflow(server, workflow, json_string);
}
...
main (int argc, char **argv) {
    ...
    char* response = register_workflow(server, workflow, json_string);
    printf ("Response: %s\n", response);
